...
then
    ...
    echo "Usage:  $0 <output_directory> <path to roboch.json>"
    exit 1
fi
...
do
    ...
    do
        for (( i=0; i<3; i++ ))
        do
            echo "Copy generated goal: goal${d}${i}.json"
            cp ./goals/goal${d}${i}.json ./goal.json
            git add ./goal.json
            git commit -m "Goal: $i, Difficulty: $d" && git push
